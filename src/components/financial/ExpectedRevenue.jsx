...
export default function ExpectedRevenue({ invoices = [] }) {
  const [expectedRevenueData, setExpectedRevenueData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [sortKey, setSortKey] = useState("remaining_expected_revenue");
  ...
  const fetchExpectedRevenue = useCallback(async () => {
    setLoading(true);
    try {
      const res = await base44.functions.invoke("getExpectedRevenue", {});
      setExpectedRevenueData(res.data);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchExpectedRevenue, 5 * 60 * 1000);
    return () => clearInterval(interval);
  }, [fetchExpectedRevenue]);

  const contracts = expectedRevenueData?.contracts ?? [];
  const summary = expectedRevenueData?.summary ?? {};
  ...
  const totalsRow = useMemo(() => ({
    ...
    remaining_expected_revenue: sorted.reduce((s, c) => s + (c.remaining_expected_revenue ?? 0), 0),
  ...
  async function handleSave() {
    ...
    if (isNew) {
      ...
      const remainingExpectedRevenue = parseFloat(form.contract_value) - totalAmount;

      if (matchingInvoices.length > 0) {
        setConfirmationMessage({
          type: "success",
          text: `Found ${matchingInvoices.length} invoice${matchingInvoices.length !== 1 ? "s" : ""} totaling ${fmt(totalAmount)} billed after ${format(new Date(backlogAsOfDate), "MMM d, yyyy")} — remaining expected revenue is ${fmt(remainingExpectedRevenue)}.`,
        });
      } else {
        setConfirmationMessage({
          type: "info",
          text: `No invoices found after ${format(new Date(backlogAsOfDate), "MMM d, yyyy")} — full contract value of ${fmt(parseFloat(form.contract_value))} is your expected revenue.`,
        });
      }
  ...
  function rowStyle(c) {
    if (c.remaining_expected_revenue < 0) return "bg-red-50 hover:bg-red-100";
    if (c.remaining_expected_revenue <= 0) return "bg-green-50 hover:bg-green-100";
    return "hover:bg-muted/50";
  }

  function billingBadge(c) {
    if (c.remaining_expected_revenue < 0) {
      return (
        <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-red-100 text-red-700">
          Over-billed {fmt(Math.abs(c.remaining_expected_revenue))}
        </span>
      );
    }
    if (c.remaining_expected_revenue === 0 || c.percent_billed >= 100) {
      return (
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={fetchExpectedRevenue} disabled={loading} className="gap-1.5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <SummaryCard label="Remaining Expected Revenue" value={fmt(summary.total_remaining_expected_revenue)} sub="Contract value – invoiced" />
        <SummaryCard label="Projected Year-End Revenue" value={fmt(summary.projected_year_end_revenue)} sub="YTD revenue + expected revenue" />
      </div>
      ...
      <div className="border rounded-xl overflow-hidden bg-card shadow-sm">
        {loading ? (
          <div className="py-12 text-center text-muted-foreground text-sm animate-pulse">Loading expected revenue…</div>
        ) : sorted.length === 0 ? (
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/50">
                  ...
                  <SortHead label="Remaining" k="remaining_expected_revenue" className="text-right" />
              ...
              <TableBody>
                {sorted.map(c => (
                  <TableRow key={c.id} className={rowStyle(c)}>
                    ...
                    <TableCell className={`text-sm text-right font-semibold ${c.remaining_expected_revenue < 0 ? "text-red-600" : ""}`}>
                      {fmt(c.remaining_expected_revenue)}
                    </TableCell>
                ...
                <TableRow className="bg-muted/70 font-semibold border-t-2">
                  ...
                  <TableCell className="text-sm text-right">{fmt(totalsRow.remaining_expected_revenue)}</TableCell>
                  <TableCell />
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 mt-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mb-2">Enter the remaining contract value as of this date. Only invoices after this date will reduce this expected revenue amount.</p>
              <Input
                type="date"
