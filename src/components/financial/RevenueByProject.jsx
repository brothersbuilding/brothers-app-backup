...
export default function RevenueByProject({ invoices, range }) {
  ...
  return (
    <div>
      <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-3">Revenue by Project (Top 10)</h2>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-card border rounded-xl p-4 shadow-sm">
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={rows} layout="vertical" margin={{ top: 0, right: 10, left: 100, bottom: 0 }}>
              ...
              <Bar dataKey="totalPaid" fill="#2563eb" name="Paid" radius={[0, 2, 2, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div className="bg-card border rounded-xl overflow-hidden shadow-sm">
          <div className="overflow-auto max-h-72">
            <Table>
              ...
              <TableBody>
                {rows.map(r => (
                  <TableRow key={r.project} className="hover:bg-muted/20">
