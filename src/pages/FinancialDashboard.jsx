...
function getComparisonRange(range, comparison) {
  ...
  switch (comparison) {
    ...
    case "previous_quarter": {
      const pqs = new Date(range.start); pqs.setMonth(pqs.getMonth() - 3);
      const pqe = new Date(range.end);   pqe.setMonth(pqe.getMonth() - 3);
...
function inRange(dateStr, range) {
  ...
  try { return isWithinInterval(parseISO(dateStr), { start: range.start, end: range.end }); }
  catch { return false; }
}
...
export default function FinancialDashboard() {
  ...
  const kpi = useMemo(() => {
    if (snapshot) {
      ...
      return {
        revenue, compRevenue, cogs, compCogs, grossProfit, compGrossProfit,
        grossMargin, compGrossMargin, labor, compLabor, opex, compOpex,
        netProfit, compNetProfit, netMargin, compNetMargin,
        revPerHead: headcount > 0 ? revenue / headcount : 0,
        compRevPerHead: headcount > 0 ? compRevenue / headcount : 0,
        projectedYearEnd, totalExpenses: cogs + labor + opex,
    ...
    return {
      ...
      revPerHead: headcount > 0 ? revenue / headcount : 0,
      compRevPerHead: headcount > 0 ? compRevenue / headcount : 0,
      projectedYearEnd, totalExpenses, compTotalExpenses,
    };
    ...
    <div className="min-h-screen bg-background">
      <div className="flex items-center justify-between px-6 py-4 border-b bg-card">
        <div>
          ...
          {lastSynced && <p className="text-xs text-muted-foreground mt-0.5">Last synced: {format(lastSynced, "MMM d, yyyy h:mm a")}</p>}
        </div>
